...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self setTitle:MJRefreshHeaderStateRefreshFailText forState:MJRefreshHeaderStateRefreshFail];
        [self setTitle:MJRefreshHeaderStateRefreshSuccessText forState:MJRefreshHeaderStateRefreshSuccess];
    }
    return self;
}
...
- (void)endRefreshingWithSuccess {
    if (self.isRefreshing) {
        self.state = MJRefreshHeaderStateRefreshSuccess;
        [self endRefreshing];
    }
}

- (void)endRefreshingWithFail {
    if (self.isRefreshing) {
        self.state = MJRefreshHeaderStateRefreshFail;
        [self endRefreshing];
    }
}
...
- (void)setState:(MJRefreshHeaderState)state
{
    ...
    switch (state) {
        case MJRefreshHeaderStateIdle: {
            ...
            } else if(oldState == MJRefreshHeaderStateRefreshSuccess || oldState == MJRefreshHeaderStateRefreshFail){
                _stateLabel.text = _stateTitles[@(oldState)];
                [UIView animateWithDuration:MJRefreshSlowAnimationDuration delay:0.0 options:UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionBeginFromCurrentState animations:^{
                    // 修复top值不断累加
                    _scrollView.mj_insetT -= self.mj_h;
                } completion:^(BOOL finished) {
                    _stateLabel.text = _stateTitles[@(state)];
                    self.state = MJRefreshHeaderStateRefreshFinish;
                }];
            }
            break;
        }
